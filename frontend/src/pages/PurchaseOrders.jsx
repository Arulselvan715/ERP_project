...
import { Plus, Check, Download, X, Eye, PlusCircle, MinusCircle } from 'lucide-react';
...
export const PurchaseOrders = () => {
  ...
  const [receiveQuantities, setReceiveQuantities] = useState({}); // { item_id: qty }
  ...
  const handleItemProductChange = (index, productId) => {
    ...
    // Auto-set the cost price of the selected product
    const productObj = products.find((p) => p.id === parseInt(productId));
    if (productObj) {
      newItems[index].price = productObj.cost_price;
    }
    setOrderItems(newItems);
  };
  ...
  const handleConfirmOrder = async (orderId) => {
    try {
      await api.post(`/purchase/${orderId}/confirm`);
      addToast('Purchase Order confirmed with supplier', 'success');
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      await api.post(`/purchase/${orderId}/cancel`);
  ...
  const openReceiveModal = () => {
    ...
    selectedOrder.items.forEach((item) => {
      // Default to remaining unreceived qty
      initialQtys[item.id] = item.quantity - item.received_qty;
    });
    setReceiveQuantities(initialQtys);
    setIsReceiveOpen(true);
  };

  const handleReceiveQtyChange = (itemId, qty) => {
  ...
  const handleReceiveSubmit = async (e) => {
    e.preventDefault();
    const receiptsPayload = Object.entries(receiveQuantities)
      .map(([itemId, qty]) => ({
    ...
    if (receiptsPayload.length === 0) {
      addToast('Please input received quantities greater than zero', 'warning');
      return;
    }

    try {
      await api.post(`/purchase/${selectedOrder.id}/receive`, receiptsPayload);
      addToast('Receipt of goods posted! Inventory increased.', 'success');
      setIsReceiveOpen(false);
      setIsDetailOpen(false);
      fetchOrders();
    } catch (error) {
      const errorMsg = error.response?.data?.detail || 'Failed to post receipt';
      addToast(errorMsg, 'error');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 font-medium">Issue requests to vendors, track shipping states, and receive shipments.</p>
        </div>
        <button
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Create New Purchase Order" size="lg">
        <form onSubmit={handleCreateOrder} className="space-y-6">
            ...
            >
              ...
              {vendors.map((v) => (
                <option key={v.id} value={v.id}>
                  {v.name} ({v.email || v.phone || 'No contact'})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
              <span className="text-sm font-bold text-slate-850 dark:text-slate-200">PO items</span>
              <button
                type="button"
                ...
                <div key={index} className="grid grid-cols-12 gap-3 items-end p-3 rounded-xl bg-slate-50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-800/50">
                  ...
                  <div className="col-span-12 sm:col-span-5">
                    ...
                    <select
                      ...
                      required
                      className="block w-full rounded-lg border border-slate-300 dark:border-slate-750 bg-white dark:bg-slate-900 px-3 py-2 text-xs text-slate-900 dark:text-white focus:border-violet-500 focus:outline-none"
                    >
                      <option value="">-- Select --</option>
                      {products.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name} ({p.sku}) - Cost: ₹{p.cost_price}
                        </option>
                      ))}
                    </select>
                  </div>
            ...
            {/* Total Cost preview */}
            <div className="flex justify-end p-2 text-sm font-bold text-slate-855 dark:text-slate-200">
              Total PO Cost: &nbsp; <span className="text-violet-600 dark:text-violet-400">{formatCurrency(calculateTotalOrderAmount())}</span>
            </div>
          </div>
      ...
      {/* PO Detail Modal */}
      <Modal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} title={`Purchase Order Detail: #${selectedOrder?.id}`} size="lg">
        <div className="space-y-6">
          {/* Top details block */}
          <div className="grid grid-cols-2 gap-4 rounded-xl bg-slate-50 dark:bg-slate-800/20 p-4 border border-slate-100 dark:border-slate-800/50 text-xs">
            <div>
              Vendor: <span className="font-bold text-slate-900 dark:text-white">{selectedOrder?.vendor_name}</span>
            </div>
            <div>
                  ...
                  {selectedOrder?.items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="p-3 text-center">
                        <span className={`font-semibold ${item.received_qty === item.quantity ? 'text-emerald-500' : 'text-slate-500'}`}>
                          {item.received_qty}
                        </span>
                      </td>
      ...
      {/* Receive Goods Modal */}
      <Modal isOpen={isReceiveOpen} onClose={() => setIsReceiveOpen(false)} title="Receive PO Goods" size="md">
        <form onSubmit={handleReceiveSubmit} className="space-y-4">
          <p className="text-xs text-slate-450 dark:text-slate-400">Input received quantities for Purchase Order #{selectedOrder?.id}</p>
              ...
              return (
                <div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-800/50">
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-[10px] text-slate-400">Remaining to receive: {remaining}</span>
                  </div>
